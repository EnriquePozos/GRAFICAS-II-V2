...
struct MatrixType
{
    ...
    D3DXVECTOR4 valores; // valores.x contendrá nuestro tiempo
};
...
public:
    SkyDome(int slices, int stacks, float radio, ID3D11Device** d3dDevice,
        ID3D11DeviceContext** d3dContext, WCHAR* texturaDiaTex, WCHAR* texturaTardeTex, WCHAR* texturaNoche)
    {
        this->slices = slices;
        this->stacks = stacks;
        this->radio = radio;
        ...
        LoadContent(texturaDiaTex, texturaTardeTex, texturaNoche);

        // Inicializar el tiempo a 0 (día)
        UpdateTime(0.0f);
    }
    ...
    {
        // timeOfDay debería estar entre 0.0 (día) y 1.0 (tarde)
        if (matrices != nullptr) {
            matrices->valores.x = timeOfDay;
        }
    }
    ...
    bool LoadContent(WCHAR* texturaDiaTex, WCHAR* texturaTardeTex, WCHAR* texturaNocheTex)
    {
        HRESULT d3dResult;
        ...
        // Cargar las dos texturas: día y tarde y noche
        d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), texturaDiaTex, 0, 0, &texturaDia, 0);
        if (FAILED(d3dResult))
        {
            return false;
        }

        d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), texturaTardeTex, 0, 0, &texturaTarde, 0);
        if (FAILED(d3dResult))
        {
            return false;
        }

        d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), texturaNocheTex, 0, 0, &texturaNoche, 0);
        if (FAILED(d3dResult))
        {
            return false;
        }
        ...
        // Inicializar valores
        ZeroMemory(matrices, sizeof(MatrixType));
    ...
    // Método Update modificado para aceptar opcionalmente un tiempo
    void Update(D3DXMATRIX view, D3DXMATRIX projection, float timeOfDay = -1.0f)
    {
        matrices->viewMatrix = view;
        matrices->projMatrix = projection;

        // Si se proporciona un tiempo válido, actualizarlo
        if (timeOfDay >= 0.0f) {
    ...
    void Render(D3DXVECTOR3 trans)
    {
        ...
        // Configurar las dos texturas
        ID3D11ShaderResourceView* textures[3] = { texturaDia, texturaTarde, texturaNoche };
        (*d3dContext)->PSSetShaderResources(0, 3, textures);
