...
public:
	TerrenoRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext)
	{
		...
		CargaParametros(L"TexturaTerreno.jpg", L"HeightmapPRUEBA14.jpg", 100.0f);
	}
	...
	bool CargaParametros(WCHAR* diffuseTex, WCHAR* heightTex, float tile)
	{
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, diffuseTex, 0, 0, &colorMap, 0 );
		 //ugsnfawlw_2K_Albedo.jpg
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"TexturaTerreno2.jpg", 0, 0, &colorMap2, 0 );
		d3dResult = D3DX11CreateShaderResourceViewFromFile( d3dDevice, L"HeightmapPRUEBA14.jpg", 0, 0, &blendMap, 0 );
